...
class BasesController extends Controller
{
    ...
    public function index(Request $request, FirebaseService $firebase)
    {
        ...
        $allBranches = false;
        if ($branchId) {
            // Use branch bases when branch is selected
            ...
            // No branch selected: list bases from all branches
            $allBranches = true;
            ...
            foreach ($bdocs as $bd) {
                ...
                $sresp = $firebase->getCollection("restaurants/{$restaurantId}/branches/{$bid}/bases");
        ...
        return view('admin.menu.bases.index', compact('bases','allBranches','branches','currentBranchId'));
    }
    ...
    public function create(Request $request)
    {
        ...
        if ($role === 'admin') {
            ...
            return view('admin.menu.bases.create', compact('restaurants','branches','selectedRestaurantId'));
        }
        if ($role === 'restaurant_admin') {
            ...
            return view('admin.menu.bases.create', compact('branches'));
        }
        // branch_admin: no selectors
        if (!$restaurantId) {
            return redirect()->route('settings.context')->with('status', 'Select restaurant first.');
        }
        return view('admin.menu.bases.create');
    }
    ...
    public function store(Request $request, FirebaseService $firebase)
    {
        ...
        $coll = $branchId
            ? "restaurants/{$restaurantId}/branches/{$branchId}/bases"
            : "restaurants/{$restaurantId}/bases";
        $id = 'base_' . Str::random(6);
        ...
        return redirect()->route('menu.bases.index')->with('status', 'Base created');
    }
    ...
    public function edit(Request $request, FirebaseService $firebase, string $id)
    {
        ...
        if ($role === 'branch_admin' && !$branchId) {
            return redirect()->route('menu.bases.index')->with('status', 'Select a branch to edit bases.');
        }
        if (!$restaurantId) {
            return redirect()->route('settings.context')->with('status', 'Select restaurant first.');
        }
        ...
        return view('admin.menu.bases.edit', compact('base'));
    }
    ...
    public function update(Request $request, FirebaseService $firebase, string $id)
    {
        ...
        if ($role === 'branch_admin' && !$branchId) {
            return redirect()->route('menu.bases.index')->with('status', 'Select a branch to update bases.');
        }
        if (!$restaurantId) {
            return redirect()->route('settings.context')->with('status', 'Select restaurant first.');
        }
        ...
        $existing = $firebase->getDocument($coll, $id);
        $f = $existing['fields'] ?? [];
        ...
        $firebase->updateDocument($coll, $id, $payload);
        return redirect()->route('menu.bases.index')->with('status', 'Base updated');
    }
    ...
    public function destroy(Request $request, FirebaseService $firebase, string $id)
    {
        ...
        $branchId = $request->input('branchId', $branchId);
        if ($role === 'branch_admin' && !$branchId) {
            return redirect()->route('menu.bases.index')->with('status', 'Select a branch to delete bases.');
        }
        if (!$restaurantId) {
            return redirect()->route('settings.context')->with('status', 'Select restaurant first.');
        }
        ...
        return redirect()->route('menu.bases.index')->with('status', 'Base deleted');
    }
}
