...
@section('content')
<h2>{{ \App\Utils\UIStrings::t('bases.create.title') }}</h2>
@if ($errors->any())
...
@endif
<form method="POST" action="{{ route('menu.bases.store') }}" class="mt-3">
  @csrf
  @isset($restaurants)
  <div class="mb-3">
    <label class="form-label">{{ \App\Utils\UIStrings::t('field.restaurant') }}</label>
    <select name="restaurantId" class="form-select" onchange="(function(sel){ if(sel.value){ window.location='{{ route('menu.bases.create') }}?restaurantId='+encodeURIComponent(sel.value); } })(this)" required>
