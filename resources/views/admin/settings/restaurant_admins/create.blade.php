...
<div class="container mx-auto p-4">
    <h1 class="text-2xl font-semibold mb-4">Create Restaurant Admin</h1>
    ...
    <form action="{{ route('settings.restaurant_admins.store') }}" method="POST" class="space-y-4 bg-white p-4 rounded shadow">
        @csrf
        <div>
        ...
        <div>
            ...
            <select name="restaurantId" class="w-full border rounded p-2" required>
                ...
                @foreach($restaurants as $r)
                    <option value="{{ $r['id'] }}" @selected(old('restaurantId')===$r['id'])>{{ $r['name'] }}</option>
                @endforeach
            </select>
        </div>
